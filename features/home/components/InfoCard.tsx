...
export const InfoCard = ({ icon, title, value, bgColor }: InfoCardProps) => (
  ...
  >
    <View className="flex-row items-center">
      {icon && <Text className="text-white text-2xl mr-1 ">{icon}</Text>}
    ...
    <Text
      className={`${!bgColor ? "text-primary dark:text-dark-secondary" : "text-foreground dark:text-dark-foreground "} text-xl font-bold`}
    >
      {value}
    </Text>
  </View>
